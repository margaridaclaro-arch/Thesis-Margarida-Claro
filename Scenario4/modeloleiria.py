...
Cortes de estrada reportados na região de Leiria nesse dia:
  - A1  cortada várias horas entre os nós de Pombal <-> Leiria e de
        Torres Novas <-> Leiria (reaberta só às 21h30)
  - A23 cortada no nó de entrada de Torres Novas
...
----------------------
Calcula, para cada freguesia do distrito de Leiria, o tempo de acesso real
por estrada (OSRM/OpenStreetMap) ao hospital do seu ULS em DOIS cenários:

  1) NORMAL   -> rede viária completa (igual ao modelo.py original)
  2) CORTADO  -> rede viária SEM AUTOESTRADAS/IP (exclude=motorway no OSRM),
                 que aproxima o efeito do corte da A1/A23 nesse dia — as
                 freguesias que normalmente usariam a A1 para chegar mais
                 depressa ao hospital passam a ser encaminhadas por estradas
                 secundárias, tal como aconteceu na realidade.
...
contexto_txt = [
    ...
    "  Castanheira de Pêra   +20 min  ->  EN236 Lousã-Castanheira de Pêra 'cortada ao trânsito' (corte total)",
    "  Leiria                +10 min  ->  IC2 zona da Roca, 'fortes constrangimentos' (via condicionada)",
    "  Marinha Grande        +10 min  ->  EN109 Leiria-Figueira da Foz, 'dificuldades de circulação'",
    "",
...
kpi_cards = [
    ('Tempo médio (normal)', f"{tempo_medio_normal} min", "2E75B6"),
    ('Tempo médio (c/ cortes)', f"{tempo_medio_cortado} min", "C00000"),
    ('Aumento médio', f"+{aumento_medio_min} min ({aumento_medio_pct}%)", "C00000"),
    ('Freguesias que passaram a críticas', f"{n_passaram_critico}/{n_freg}", "C00000"),
]
card_cols = ['B', 'D', 'F', 'H']
...
ws_dash['B8'] = (f"Críticas (>60min) antes: {n_criticas_normal}  |  depois do corte: {n_criticas_cortado}  |  "
                 f"população nas freguesias que pioraram: {pop_passaram_critico:,} de {pop_total:,} hab.")
